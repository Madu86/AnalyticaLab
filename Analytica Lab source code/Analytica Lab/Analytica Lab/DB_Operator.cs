...
namespace Analytica_Lab
{
    class DB_Operator
    {
        ...
        public void BackUPDB(string path) {
            ...
            File.Copy("Analytica_Lab.mdf",path);
            
        
        }

        //returns compound_id column and a specified reagent column as a table... 
        public DataTable GetDataFromChemicalCompoundsTable(string reagentCode) {

            
            cmd = new SqlCommand("SELECT Compound_ID, " + reagentCode + "  FROM Chemical_Compounds", con);
            da = new SqlDataAdapter(cmd);
            ds = new DataSet();

            con.Open();
            da.Fill(ds,"Chemical_Compounds");
            con.Close();
